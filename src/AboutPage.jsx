...
import { Heart, X, ExternalLink, MapPin, Tag } from 'lucide-react';

// Footer Components
const FooterButton = ({ title, isActive, onClick }) => {
  return (
    <button
      onClick={onClick}
...
const Footer = ({ activePage, onNavigate }) => {
  return (
    <div className="flex flex-row items-center justify-around bg-black py-1">
      <FooterButton title="Home" isActive={activePage === 'home'} onClick={() => onNavigate('home')} />
      <FooterButton title="Likes" isActive={activePage === 'likes'} onClick={() => onNavigate('likes')} />
      <FooterButton title="My Matches" isActive={activePage === 'matches'} onClick={() => onNavigate('matches')} />
      <FooterButton title="Profile" isActive={activePage === 'profile'} onClick={() => onNavigate('profile')} />
    </div>
  );
};
...
const LikesScreen = () => {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen w-screen bg-neutral-900 relative">
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <p className="text-sm text-neutral-600 mt-1">
                  {currentLike.size} | Woman | {currentLike.location}
                </p>
              </div>
              <span
